defmodule Inventario do
  ...
  defp menu(inventario) do
    ...
    case opcion do
      ...
      4 ->
        inventario |> actualizar() |> menu()

      5 ->
        inventario |> eliminar() |> menu()

      6 ->
        Util.mostrar_mensaje("Has salido del inventario")
  ...
  defp buscar(inventario) do
    referencia =
      "Nombre de la referencia a buscar: "
      |> Util.ingresar(:texto)
  ...
  defp actualizar(inventario) do
    referencia =
      "Nombre de la referencia a actualizar: "
      |> Util.ingresar(:texto)
      ...
      {ref, talla, precio, colores} ->
        ...
        producto_actualizado =
          case opcion do
            ...
            3 ->
              nuevos_colores =
                "Nuevos colores (separados por coma): "
                |> Util.ingresar(:texto)
                |> String.split(",")
                |> Enum.map(&String.trim/1)
                |> ingresar_unidades_color()
  ...
  defp eliminar(inventario) do
    referencia =
      "Nombre de la referencia a eliminar: "
      |> Util.ingresar(:texto)
